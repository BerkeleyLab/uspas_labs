#!/usr/bin/env python3
# modified from: https://github.com/ttrftech/NanoVNA/blob/master/python/nanovna.py
import serial
...
class NanoVNA:
    def __init__(self, dev = None):
        ...
        try:
            self.send_command("version\r")
            self.version = version.parse(self.fetch_data().strip())
        except version.InvalidVersion as e:
            print(f"{e}: May occur when first booted. Please try again.")
            raise
        else:
    ...
    def set_port(self, port):
        if port is not None:
            if self.version == version.Version("1.1.01"):
                self.send_command("trace %d\r" % port)
